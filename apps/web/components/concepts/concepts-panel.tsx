  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {canGenerate ? (
        <p className="text-[15px] leading-relaxed text-ink-2">
          {!hasPhoto
            ? 'Создадим интерьер по типу комнаты, размерам и описанию окон и дверей. Сверьте мерки перед запуском; фото даст дополнительный ориентир для вида комнаты.'
            : keepsFurniture
              ? 'Фото станет основой концепта: в задании сохраняем текущую обстановку и меняем только то, что указано в заметках. В готовом варианте сверьте важные детали с фото.'
              : 'Фото задаст ракурс и архитектуру комнаты. Предложим новую отделку и мебель; в готовом варианте сверьте положение окон и дверей с исходником.'}{' '}
          Концепт помогает выбрать стиль; размеры и размещение проверяются отдельно на 2D-схеме.
        </p>
      ) : null}
