...
const linkClassName =
  'text-[15px] text-ink-2 underline-offset-[6px] decoration-1 decoration-accent transition-colors duration-200 ease-ui hover:text-ink hover:underline'

export function SiteHeader({ theme, user }: { theme: Theme; user: { name: string } | null }) {
  return (
    <header className="border-b border-line">
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between gap-6 px-5 sm:px-8">
        <Link href="/" className="font-serif text-[26px] leading-none tracking-tight text-ink">
          Uyut
        </Link>
