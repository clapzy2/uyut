...
/**
 * Рулон ковролина или дорожка на отрез: в названии стоит длина в тысячах сантиметров.
 * Такое в интерьер не поставишь, а по слову «ковёр» оно лезет первым.
 */
const SOLD_BY_THE_METRE = /\d{2,4}\s*[хx×*]\s*\d{4,}/
 ...
 */
const NOT_FURNITURE =
  /чехол|накидк|наматрасник|еврочехол|обивочн|ремкомплект|запчаст|фурнитур|ножк[аи]\b|опор[аы] для|подлокотник|механизм трансформац|наклейк|фотообои|обои\b|духов|холодильн|морозильн|посудомоеч|стиральн|сушильн|вытяжк|варочн|микроволнов|для кукол|кукольн|миниатюр|игрушечн|конструктор|пазл|макет\b|лампочк|лампа накаливания|светодиодная лампа|лампа led|цокол|патрон|филамент|лента светодиодн|блок питания|драйвер|выключател|розетк|удлинител|коврик для мыш|коврик для ванн|коврик для йог|коврик придверн|автоковр|в багажник|багажник|в салон автомобил|под умывальник|для ванной комнаты|под раковин|для кошек|для собак|для животных|лежанк|когтеточ|столов[ыа][ея] прибор|столешниц|подстолье|бильярдн|теннисн|верстак|уценк|б\/у/i
...
function dimensions(text: string): { width: number | null; depth: number | null; height: number | null } {
  ...
  if (triple) {
    return { width: sane(triple[1] ?? ''), depth: sane(triple[2] ?? ''), height: sane(triple[3] ?? '') }
  }
